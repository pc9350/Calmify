...
const TeamPage = () => {
    ...
    return (
        <section className="py-14 bg-gradient-to-b from-sage-green-light to-white">
            <motion.div 
            ...
            >
                <div className="text-center">
                    <motion.h3 
                        className="mt-8 text-lime-700 text-3xl font-bold sm:text-4xl"
                        initial={{ scale: 0.5 }}
                        animate={{ scale: 1 }}
                ...
                <div className="mt-12">
                    <ul className="grid gap-8 lg:grid-cols-2">
                        {team.map((item, idx) => (
                            <motion.li 
                                key={idx}
                                className="gap-8 sm:flex bg-white rounded-xl shadow-lg overflow-hidden"
                                initial={{ opacity: 0, x: -50 }}
                                animate={{ opacity: 1, x: 0 }}
                                transition={{ delay: idx * 0.2 }}
                            >
                                <div className="w-full sm:w-2/5 h-60 relative">
                                    <Image
                                        src={item.avatar}
                                        layout="fill"
                                        objectFit="cover"
                                ...
                                <div className="p-4 sm:w-3/5 flex flex-col justify-between">
                                    <div>
                                        <h4 className="text-xl text-gray-700 font-semibold">{item.name}</h4>
                                        <p className="text-lime-700 font-medium">{item.title}</p>
                                        <p className="text-gray-600 mt-2 text-sm">{item.desc}</p>
                                    </div>
                                    <div className="mt-4 flex gap-4 text-gray-400">
                                        <a href={item.github} target="_blank" rel="noopener noreferrer" className="transition-colors duration-300 hover:text-gray-800">
                                        ...
                                        <a href={item.linkedin} target="_blank" rel="noopener noreferrer" className="transition-colors duration-300 hover:text-gray-800">
                                            <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
